* types & grammar
  ...
  2. 如何逆序一个字符串？function reverse(str) {
  ...
  4. 判断以下结果，为什么会出现这样的情况，如何做出正确的比较？
    ...
    false，false
  5. 如何判断一个数值为整数？
var numberLen1=number1.toString().split("").length;
var numberLen2=parseInt(number1).toString().split("").length;
if(numberLen1==number2){
console.log("整数")
}else
{
console.log("小数")
